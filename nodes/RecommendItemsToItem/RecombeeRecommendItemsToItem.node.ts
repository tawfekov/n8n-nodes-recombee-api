...
export class RecombeeRecommendItemsToItem implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee RecommendItemsToItem',
		name: 'recombeeRecommendItemsToItem',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'RecommendItemsToItem operation from Recombee with batching',
		defaults: {
		...
		for (let i = 0; i < items.length; i += batchSize) {
			...
			const batchPromises = batchItems.map(async (item, index) => {
				...
				try {
					...
					const request = new requests.RecommendItemsToItem(itemId, userId, count, { scenario });
					request.timeout = timeout;

					const data = await sendWithRetry(request, maxRetries);
					return { json: { success: true, itemId, userId, count, scenario, data } };
				} catch (error) {
