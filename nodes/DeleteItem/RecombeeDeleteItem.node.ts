...
export class RecombeeDeleteItem implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee DeleteItem',
		name: 'recombeeDeleteItem',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'DeleteItem operation from Recombee',
		defaults: {
			name: 'DeleteItem',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				const request = new requests.DeleteItem(itemId);
