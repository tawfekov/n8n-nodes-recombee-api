 ...
 */
export function toEpochTimestamp(value: unknown): string {
	if (value === undefined || value === null) {
		return Math.floor(Date.now() / 1000).toString();
	}
	const date = value instanceof Date ? value : new Date(value as any);
	return isNaN(date.getTime()) ? Math.floor(Date.now() / 1000).toString() : Math.floor(date.getTime() / 1000).toString();
}
