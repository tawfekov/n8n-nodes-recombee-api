...
export class RecombeeAddUser implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee AddUser',
		name: 'recombeeAddUser',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'Creates a new user in your Recombee database. Users are specified using unique string identifiers that can contain digits, Latin letters, underscores, colons, minus signs, at signs, and dots. This operation is essential for building user profiles and enabling personalized recommendations.',
		defaults: {
			name: 'AddUser',
		},
		...
		properties: [
			{
				...
				default: '',
				required: true,
				description: 'Unique identifier for the user. Can contain digits, Latin letters, underscores, colons, minus signs, at signs, and dots.',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				const request = new requests.AddUser(userId);
