...
export class RecombeeGetUserValues implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee GetUserValues',
		name: 'recombeeGetUserValues',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'GetUserValues operation from Recombee',
		defaults: {
			name: 'GetUserValues',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				const request = new requests.GetUserValues(userId);
