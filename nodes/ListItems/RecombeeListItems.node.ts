...
export class RecombeeListItems implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee ListItems',
		name: 'recombeeListItems',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'ListItems operation from Recombee',
		defaults: {
			name: 'ListItems',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				const request = new requests.ListItems();
