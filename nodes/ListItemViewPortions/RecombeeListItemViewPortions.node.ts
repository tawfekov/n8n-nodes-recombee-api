...
export class RecombeeListItemViewPortions implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee ListItemViewPortions',
		name: 'recombeeListItemViewPortions',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'Lists the viewed portions of an item (for example a video or article).',
		defaults: {
			name: 'ListItemViewPortion',
		},
		...
		properties: [
			{
				...
				default: '',
				required: true,
				description: 'The ID of the item to list its portions',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const processedItems: { itemId: string; index: number }[] = [];
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				const request = new requests.ListItemViewPortions(itemId);
