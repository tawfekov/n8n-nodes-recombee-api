...
export class RecombeeDeleteUser implements INodeType {
	description: INodeTypeDescription = {
		usableAsTool: true,
		displayName: 'Recombee DeleteUser',
		name: 'recombeeDeleteUser',
		icon: 'file:../RecombeeNode.svg',
		group: ['transform'],
		version: 1,
		description: 'DeleteUser operation from Recombee',
		defaults: {
			name: 'DeleteUser',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			for (let i = 0; i < items.length; i++) {
				...
				const request = new requests.DeleteUser(userId);
